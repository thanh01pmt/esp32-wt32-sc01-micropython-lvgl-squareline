...
class st7796(ili9XXX):
    ...
    # The st7796 display controller has an internal framebuffer arranged in a 480x320 pixel
    # configuration. Physical displays with pixel sizes less than 480x320 must supply a start_x and
    # start_y argument to indicate where the physical display begins relative to the start of the
    # display controllers internal framebuffer.

    def __init__(self,
        miso=-1, mosi=13, clk=14, cs=15, dc=21, rst=22, power=-1, backlight=23, backlight_on=1, power_on=0,
        spihost=esp.HSPI_HOST, spimode=0, mhz=80, factor=16, hybrid=True, width=480, height=320, start_x=0, start_y=0,
        colormode=COLOR_MODE_RGB, rot=REVERSE_LANDSCAPE, invert=True, double_buffer=False, half_duplex=True,
        asynchronous=False, initialize=True, color_format=lv.COLOR_FORMAT.NATIVE_REVERSE):
...
#                 self.madctl(colormode, rot, (0x0, MADCTL_MX | MADCTL_MV, MADCTL_MY | MADCTL_MX, MADCTL_MY | MADCTL_MV))])},  # MADCTL
            ...
            {'cmd':  0xe8, 'data': bytes([0x40, 0x8a, 0x00, 0x00, 0x29, 0x19, 0xa5, 0x33])},
